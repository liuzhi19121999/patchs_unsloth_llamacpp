...
class GRPOTrainer(Trainer):
    ...
    ## My Code to Use LLama.cpp
    def run_llama_cpp(self, model_path = "./Llama/models/temp.gguf", batch = 1):
        from subprocess import Popen, DEVNULL
        from time import sleep
        max_length = (self.args.max_prompt_length + 150) * batch
        cmd_line = f"./Llama/llama-server.exe -m {model_path} -np {batch} -fa --port 8100 --no-mmap -c " + str(max_length) + " -ngl 99"
        cmd_line = cmd_line.split(' ')
        self.process = Popen(cmd_line, stdout=DEVNULL, stderr=DEVNULL)
        sleep(3)
    ...
    def covert_to_gguf(self):
        from patchs_unsloth_llamacpp import generate_model_for_acc
        self.model.save_pretrained_merged("./temp", self.processing_class, save_method = "merged_16bit",)
        generate_model_for_acc("./temp", "./Llama/models/temp.gguf")
    
    def generate_complations(self, prompts: list[str]):
        from json import loads
        from requests import post, get
        from time import sleep
        while True:
            try:
                resp = get("http://localhost:8100//health", timeout=10)
                if loads(resp.content.decode())["status"] == "ok":
                    break
            except Exception:
                pass
            sleep(1)
        while True:
            try:
                resp = post("http://localhost:8100/completion", json={
                    "prompt": prompts, "temperature": self.args.temperature,
                    "n_predict": self.args.max_completion_length,
                    "top_p": 0.95, "top_k": 40}, timeout=600)
                break
            except Exception:
                sleep(2)
        complations = [i["prompt"] + i["content"] for i in loads(resp.content.decode())]
        return torch.tensor(self.processing_class(complations, padding='longest')["input_ids"]).to("cuda:0")
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.args.use_vllm:
            ...
            if self.process == None:
                self.covert_to_gguf()
                self.run_llama_cpp()
            prompt_completion_ids = self.generate_complations(prompts_text)
...
def patch(typename: str = "GRPO"):
    ...
    if not exists("./Llama/llama-server.exe"):
        print("Error : llama-server.exe Not Found\nAs llama.cpp is an open source project, please download suitable llama.cpp version from https://github.com/ggml-org/llama.cpp into directory ./Llama")
        exit(0)
...
def unsloth_cpu_oom_patch(is_cpu_oom: bool = False):
    ...
    if is_cpu_oom:
        is_use_cuda_memory(is_cpu_oom)
    write_file_content(Path(gradient_checkpointing.__file__).absolute().__str__(), unsloth_zoo_content)
    print("Patch unsloth cpu oom success! You can save data in VRAM instead of RAM")
